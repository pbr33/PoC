...
class AzureAI:
    ...
    def generate_ai_arch_svg(self, arch: dict, semantic: dict) -> "str | None":
        ...
        SYSTEM = (
            "You are an expert enterprise architecture diagram designer. "
            "Create a professional, publication-quality architecture diagram as a COMPLETE self-contained HTML page. "
            "\n\nSTRICT OUTPUT FORMAT:"
            "\n- Return ONLY valid HTML starting with <!DOCTYPE html> and ending with </html>"
            "\n- No markdown fences, no explanation, no text outside HTML"
            "\n- 100% self-contained — zero external CDN/font/image resources"
            "\n\nVISUAL STYLE (match enterprise whitepaper aesthetic):"
            "\n- Background: white #FFFFFF, body font: 'Segoe UI',system-ui,Arial,sans-serif"
            "\n- Clean, professional, light-theme — similar to Microsoft/Azure architecture whitepapers"
            "\n- NOT a dark neon diagram — professional document style"
            "\n- Drop shadows: box-shadow:0 2px 8px rgba(0,0,0,.08)"
            "\n- Rounded corners: border-radius:8px on section boxes, 6px on cards"
            "\n\nCOLOR PALETTE:"
            "\n- Primary blue: #0078D4 (headers, main borders, primary arrows)"
            "\n- Light blue fill: #EBF5FB (AI/interface sections)"
            "\n- Middleware fill: #EDE7F6 border:#5C2D91 (orchestration layer)"
            "\n- Data layer fill: #E8F5E9 border:#107C10"
            "\n- Foundation fill: #FFF9E6 border:#FFB900 (yellow Azure foundation)"
            "\n- Sidebar fill: #F8FAFC border:#E2E8F0 (cross-cutting, outcomes)"
            "\n- Future/dashed: #F9FAFB border:#9CA3AF dashed"
            "\n- Body text: #1F2937; Subtext: #6B7280; White text on dark headers: #FFFFFF"
            "\n- Outcome checkmarks: #107C10"
            "\n\nLAYOUT (3-column + foundation row + legend):"
            "\n- Outer container: max-width:1200px, margin:auto, padding:24px"
            "\n- TITLE ROW: bold 22px title centered, italic 13px subtitle below"
            "\n- MAIN GRID: 3 columns: left 18% | center 62% | right 20%"
            "\n  LEFT COL: 'Users' small box (top) + 'Knowledge Sources / Input Systems' box (below)"
            "\n  CENTER COL (stacked top→bottom with arrows between):"
            "\n    1. AI Interface Layer box (blue header, service cards inside)"
            "\n    2. Middleware Orchestration Layer box (purple header, capability cards in a row)"
            "\n    3. Enterprise Data & Search Layer box (green header, 2 sub-boxes side-by-side)"
            "\n  RIGHT COL: Future AI Platforms (dashed) + Cross-Cutting Components + Business Outcomes + Key Design Principles"
            "\n- FOUNDATION ROW: full-width yellow box 'Azure Foundation' with 4 service boxes side-by-side"
            "\n- LEGEND ROW: small legend box bottom-left with 4-5 arrow types"
            "\n\nSECTION HEADERS: colored band (border-radius top 8px), white text, bold 12px uppercase"
            "\nSERVICE CARDS: 130px wide, white bg, 1px border, padding 10px, emoji icon 18px + name 11px bold + type 9px gray"
            "\nARROWS: Use CSS-drawn arrows or SVG overlay. Solid blue=user/query, solid purple=data/retrieval, green=monitoring, dashed=future"
            "\nLabel key arrows with small text badges"
            "\nBUSINESS OUTCOMES: checkmark list (✓ green) with 5-6 outcomes"
            "\nKEY DESIGN PRINCIPLES: bullet list with 4-5 principles, 11px text"
            "\nHOVER: .svc-card:hover{box-shadow:0 4px 14px rgba(0,120,212,.18);transform:translateY(-2px);transition:all .2s}"
        )

        USER = (
            f"Generate a complete HTML architecture diagram for this solution:\n\n{arch_str}\n\n"
            "REQUIREMENTS:\n"
            "1. Use the exact 3-column + foundation layout described\n"
            "2. Show ALL components from the JSON in the appropriate layer\n"
            "3. Draw visible arrows: left-to-center (user flow), center vertical (data flow), center-to-right (monitoring)\n"
            "4. Foundation row shows Azure infrastructure services from the components\n"
            "5. Business Outcomes right panel: list 5-6 business benefits of this architecture\n"
            "6. Cross-Cutting Components: security, identity, monitoring, API management\n"
            "7. Key Design Principles: 4-5 architectural decisions/patterns used\n"
            "8. Flow Legend at bottom\n"
